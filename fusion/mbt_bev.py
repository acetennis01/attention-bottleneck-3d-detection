...
class SymmetricMBTBEVFusion(nn.Module):
    ...
    def forward(
        ...
        for image_block, lidar_block in zip(
            ...
            image_block_output = image_block(
                torch.cat((image_tokens, bottlenecks), dim=1),
                return_attention=return_attention,
            )
            lidar_block_output = lidar_block(
                torch.cat((lidar_tokens, bottlenecks), dim=1),
                return_attention=return_attention,
            )
            if return_attention:
                image_updated, image_attention = image_block_output
                lidar_updated, lidar_attention = lidar_block_output
                # Retain only bottleneck-query to spatial-token attention.
                image_bottleneck_attention.append(
                    image_attention[
                        :, :, image_length:, :image_length].detach())
                lidar_bottleneck_attention.append(
                    lidar_attention[
                        :, :, lidar_length:, :lidar_length].detach())
            else:
                image_updated = image_block_output
                lidar_updated = lidar_block_output
