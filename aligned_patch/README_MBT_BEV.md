...
## Calibration-aware camera-to-BEV follow-up

`my_fusion_mbt_bev_aligned_eb48.py` is the new geometry-aligned experiment.
For every 16x16 LiDAR BEV cell, it projects 3D queries at four heights through
the sample's KITTI `lidar2img` calibration, applies the recorded image resize,
bilinearly samples the ResNet feature map, and masks cells outside the camera
view. A learned height weighting and local gate inject visual content into the
matching LiDAR token before the symmetric MBT layers. The original unaligned
configuration is retained so its published checkpoint remains reproducible.

Run the bounded two-update check first:

```bash
python tools/train.py \
  projects/myfusion/configs/my_fusion_mbt_bev_aligned_eb48_smoke.py
```

Then train the aligned model from scratch:

```bash
python tools/train.py \
  projects/myfusion/configs/my_fusion_mbt_bev_aligned_eb48.py
```

The diagnostics report `fusion/image_valid_ratio`,
`fusion/local_camera_gate_mean`, `grad_norm/fusion_alignment`, and
`grad_norm/image_backbone`. After selecting the best checkpoint, evaluate it
with the normal, black-image, and shuffled-image configurations. Meaningful
fusion requires the correct-image result to exceed both ablations by more than
ordinary seed-to-seed variation.

### Camera-focused MBT follow-up

The first aligned run still produced effectively identical correct, black,
and shuffled-image AP40 at its epoch-32 checkpoint. The new
`my_fusion_mbt_bev_camera_focused_eb48.py` experiment therefore keeps the
geometric alignment but makes the shared attention bottlenecks the only
camera-to-LiDAR semantic path. It adds camera-covered LiDAR-token dropout,
mild LiDAR-BEV channel dropout, and a class-specific camera-BEV center loss
after bottleneck readout. It warm-starts from the best LiDAR-only checkpoint,
freezes the LiDAR encoder and detection head for five epochs, then fine-tunes
all modules with lower LiDAR learning rates.
...
This is a 40-epoch experiment. The corresponding black and shuffled configs
are `my_fusion_mbt_bev_camera_focused_black_eval.py` and
`my_fusion_mbt_bev_camera_focused_shuffled_eval.py`.

## Best-checkpoint validation results

The results below belong to the original, spatially unaligned MBT experiment;
they are not results for the new calibration-aware model.
